...
### 📝 现代替代方案：`#pragma once`（教材外拓展）

实际工程中，越来越多项目用 **`#pragma once`** 代替头文件保护——写在头文件**第一行**，告诉编译器"这个文件在一次编译中最多包含一次"。

**两种写法对比**：

```cpp
// 头文件保护：3 行，还要起一个全局唯一的宏名
#ifndef SALES_DATA_H
#define SALES_DATA_H
struct Sales_data { /* ... */ };
#endif

// #pragma once：1 行，不依赖任何宏名
#pragma once
struct Sales_data { /* ... */ };
```

**为什么流行**：
- **不用起宏名**：彻底消除"两个头文件撞宏名"的隐患（预处理器变量必须全局唯一，大型项目里重名是真实的 bug 来源）
- **重构友好**：文件改名、类改名时无需同步修改宏名
- **更简洁**：一行替代三行模板代码

**局限**：
- **不在 C++ 标准中**：它是编译器扩展，ISO 标准没有定义这个指令——这正是 Primer 没有收录它的原因
- 理论上的边界情况：同一文件通过不同路径或链接被包含时，老旧编译器可能识别不出是同一个文件（现代主流编译器已基本解决）

**工程现状**：
- GCC、Clang、MSVC 等**主流编译器全部支持**，已是**事实标准**(de facto standard)
- MSVC 新建头文件的模板默认自带 `#pragma once`
- 追求绝对可移植性的项目（如某些嵌入式工具链）仍使用头文件保护；也有项目两者都写求双保险

🌟 **选择建议**：
- 考试、教材语境 → 头文件保护（标准内容，必须掌握）
- 现代项目实践 → `#pragma once` 优先，除非目标平台有不支持它的编译器
- 同一项目里**遵循既有约定**，不要两种风格混用
