...
* [*内联函数的现代视角*](#id6)：手写inline对性能已无意义，其价值只剩链接语义（教材外拓展）
* [*constexpr函数*](#id7)：用于常量表达式的函数，编译时求值
* [*assert预处理宏*](#id8)：assert宏和预定义调试宏
* [*NDEBUG控制与预定义宏*](#id9)：NDEBUG控制与预定义宏
...
<a id="id6"></a>
## ✅ 知识点7: 现代视角——`inline` 的性能意义已消退

**理论**
* **一句话结论**："内联展开"这种优化**没有过时**——过时的是**程序员手写 `inline` 来优化性能**这件事，这项工作已被现代编译器完全接管
* **原因1：编译器拥有绝对的决定权（双向忽略）**
    * **写了也不一定内联**：函数体太大、含循环或是递归时，编译器直接无视 `inline`，按普通函数编译
    * **不写也会自动内联**：开启常规优化（`-O2`/`-O3`）后，编译器自行分析哪些函数值得内联——短小的函数即使不加关键字也会被自动内联
    * 📋 早期编译器优化能力弱，倾向于照字面采纳 `inline` 建议——所以它给人的感觉逐渐从"指令"退化成了纯建议
* **原因2：链接时优化（LTO）打破了文件壁垒**
    * **过去**：编译器只能看到单个编译单元——函数定义在 A.cpp、在 B.cpp 被调用时，编译 B.cpp 看不到函数体，无法内联，因此需要"头文件 + `inline`"这套组合拳
    * **现在**：主流编译器支持 **LTO**（Link Time Optimization，链接时优化），链接阶段重新拿到全局代码的中间表示，可以**跨文件内联**——函数定义在源文件里、不加 `inline` 也能被内联到调用处
* **原因3：启发式算法比人类更懂权衡**
    * 内联是**双刃剑**：省去调用开销，但导致代码体积膨胀；过度内联会破坏**指令缓存**(I-cache)，缓存未命中的损失远大于省下的调用开销
    * 编译器内置复杂的启发式阈值（如 GCC 的 `-finline-limit`、`inline-threshold` 等参数），综合评估函数长度、调用频率、是否在热循环、寄存器压力——数据驱动的决策远比程序员凭直觉写一个 `inline` 精准
* **`inline` 今天的真正价值——链接语义，而非优化**
    * **允许跨编译单元的多重定义**：`inline` 函数可以在多个编译单元中各有一份（必须相同的）定义而不违反**单一定义规则**(ODR)——这才是"在头文件里定义函数"的真正通行证，也是 `inline` 今天的核心价值
    * **C++17 `inline` 变量**：变量也获得了同样的"多重定义豁免"，头文件里可以直接定义全局常量，不再需要变通写法

**示例/实践**
```bash
# 开启 LTO：编译与链接都加 -flto，跨文件内联自动发生
g++ -O2 -flto -c a.cpp -o a.o
g++ -O2 -flto -c b.cpp -o b.o
g++ -flto a.o b.o -o prog
```
```cpp
// 真要强制内联（极少数场景，如 SIMD intrinsics 包装）
__attribute__((always_inline)) inline int add(int a, int b) { return a + b; }  // GCC/Clang
__forceinline int add(int a, int b) { return a + b; }                          // MSVC

// C++17：头文件里直接定义全局常量，无需任何变通
inline constexpr int max_size = 1024;   // inline变量，多编译单元共享一份
```

**注意点**
> ⚠️ **不要滥用强制内联**：`always_inline`/`__forceinline` 剥夺了编译器的权衡权，仅在编译器"看不见"内联必要性的场景（如 intrinsics 包装）使用
> 💡 **`inline` 的隐含语义**：同一个 `inline` 函数在所有编译单元中**地址唯一**，其函数体内的局部 `static` 变量全程序也只有一份（链接器负责合并）——普通函数若误放进头文件，则每个编译单元各有一份，行为完全不同
> 🔄 **呼应 7.6**：知识点4 讨论的"取地址就必须类外定义"是 C++11/14 时代的约束；C++17 起 `static constexpr` 成员隐式为 `inline`，类外定义彻底成为历史（见 [7.6 知识点4](../Chapter07_classes/note_7_6_static_class_member.md#id8)）
> 🔄 **与教材的关系**：教材说"inline 只是建议，编译器可能忽略"——现代实践把这句话推到极致：不仅"可能"忽略，而是**总是**由编译器说了算；手写 `inline` 的动机只剩链接语义
