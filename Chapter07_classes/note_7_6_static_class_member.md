...
* [*知识点6: 静态关键字的可见性*](#id10)：静态关键字还会改变可见性
...
**注意点**
* ⚠️ 静态成员函数**不能直接**调用非静态成员（缺乏`this`指针），但可以通过一个对象（如形参）间接调用
...
r = ac3.rate();    // 通过Account引用
r = ac2->rate();   // 通过Account指针
...
* 声明后若被使用但**未在类外定义**，会在**链接阶段**报"未定义引用"(undefined reference)错误
...
**理论**
* 静态数据成员类似全局对象：整个程序只有一份，存在于整个程序生命周期（程序结束才销毁）——因此其定义**必须出现在函数之外的命名空间作用域**
* **一般情况下**必须在类外定义和初始化静态数据成员（例外见知识点4：const 整型 / constexpr）
* 定义的方式和函数定义相似：
...
**注意点**
* ⚠️ **最佳实践**：把 static 成员的定义放在和非内联成员函数同一个`.cpp`文件里。
    * 非内联成员函数一般都写在对应的 `.cpp`中，
    * 因为它们的定义只能出现一次，而 `.h` 文件可能被多个`.cpp`包含，会引发多重定义错误。
    * 静态成员也是这个道理。
...
```
```cpp
int x = Account::period;      // OK：纯值替换，标准不要求定义（最佳实践仍建议类外定义）
...
* 静态成员**独立**于任何其他对象而存在
* 可以以非静态数据成员不允许的方式使用：
    * 静态数据成员可以有**不完整**类型(详见 [7.3 知识点12](./note_7_3_additional_class_features.md#id12))
...
**理论**
- 在C/C++中，`static` 关键字的核心作用之一就是**控制和限制可见性**。它的具体表现取决于其修饰的对象：

    **1. 修饰全局变量与普通函数（文件作用域）**
    * **默认情况**：未加 `static` 的全局变量和函数具有外部链接性，对整个工程的所有源文件可见。
    * **static 作用**：将其链接属性从外部链接改为内部链接。被 `static` 修饰的全局变量或函数仅在**定义它们的源文件（编译单元）内可见**，对其他源文件完全隐藏。
    * **意义**：实现了文件级别的封装。不同源文件可以定义同名的 `static` 变量或函数，而不会发生链接冲突。

    **2. 修饰局部变量（函数/块作用域）**
    * **可见性不变**：`static` **不改变**局部变量的可见性。它的作用域仍然仅限于定义它的函数或语句块内部，外部函数无法访问。
    * **实质改变**：虽然可见性没变，但 `static` 将其存储位置从栈移到了静态存储区，使其生命周期延长至整个程序运行期间。即变量在函数调用结束后不会销毁，下一次调用时仍保留上一次的值，但其他函数依然看不见它。

    **3. 修饰类成员（C++ 类作用域）**
    * **静态数据成员**：属于类本身而非任何一个对象实例。在C++17之前，非 `const` 的静态数据成员需要在类外（全局作用域或命名空间作用域）进行定义，其可见性受类的访问修饰符（`public`、`private`、`protected`）严格控制。在类外定义时，不再需要 `static` 关键字。
    * **静态成员函数**：同样属于类，没有 `this` 指针，只能**直接**访问类的静态成员（通过一个对象可以间接访问非静态成员）。其可见性同样受访问修饰符限制，可以通过类名直接调用。

- **对于全局变量和函数**，`static` 起到“**隐藏**”作用，将可见性缩小到单个文件内
- **对于局部变量**，`static` 保持原有可见性不变，仅延长其生命周期
- **对于类成员**，`static` 使其脱离对象实例绑定，可见性交由类的访问控制权限管理。
...
6. **可见性控制**：`static` 对全局实体缩小链接范围（内部链接），对局部变量只延长生命周期，对类成员交给访问控制管理
