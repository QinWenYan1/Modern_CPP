...
* 默认使用`<`运算符确定相对优先级, 也就是`less<>`
  * 其中运算符 `compare(a, b) = true` 表示`a`的优先级比`b`低（`b` 会在堆顶）
  * `std::less` 调 `operator<`，`std::greater` 调 `operator>`
* 传递 `less<>`表示 **最大堆**：每个**父节点都 ≥ 子节点**，因此堆顶一定是最大值
* 传递 `greater<>`表示 **最小堆**：每个**父节点都 ≤ 子节点**，因此堆顶一定是最小值
